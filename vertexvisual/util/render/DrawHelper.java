...
import dev.sxmurxy.vertexvisual.util.misc.ColorHelper;
import dev.sxmurxy.vertexvisual.util.misc.Utils;
...
public class DrawHelper implements Wrapper {
    ...
    public static void drawTriangle(double v, double v1, double v2, double v3, double v4, double v5, Color arrowColor) {
    }
    ...
    public static void drawQuad(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, Color color) {
        drawSetup();
        applyColor(color);
    ...
    public static void drawText(String text, double x, double y, Color color) {
        ...
        // или fallback на стандартный, если шрифт не загружен
    }
    ...
    public static void drawColorWheel(double x, double y, double size, Color currentColor, double currentAngle, double currentRadius, double animation) {
        ...
        // Оптимизированная отрисовка цветового круга
        int angleStep = 3; // шаг угла в градусах для оптимизации
        int radiusStep = 2; // шаг радиуса для оптимизации
        ...
        for (int angle = 0; angle < 360; angle += angleStep) {
            for (int r = 0; r < radius; r += radiusStep) {
                ...
                // Текущий и следующий углы в радианах
                double rad1 = Math.toRadians(angle);
                double rad2 = Math.toRadians(angle + angleStep);

                // Координаты текущего сектора
                double x1 = centerX + r * Math.cos(rad1);
                double y1 = centerY + r * Math.sin(rad1);
                double x2 = centerX + (r + radiusStep) * Math.cos(rad1);
                double y2 = centerY + (r + radiusStep) * Math.sin(rad1);
                double x3 = centerX + (r + radiusStep) * Math.cos(rad2);
                double y3 = centerY + (r + radiusStep) * Math.sin(rad2);
                double x4 = centerX + r * Math.cos(rad2);
                double y4 = centerY + r * Math.sin(rad2);

                // Цвет для текущего сектора (HSV -> RGB)
                Color color = Color.getHSBColor((float)angle / 360f, (float)progress, 1f);
                applyColor(color);
        ...
        if (animation > 0) {
            ...
            // Белый кружок
            applyColor(new Color(255, 255, 255, (int)(255 * animation)));
            drawRoundedRect(markerX, markerY, markerRadius, markerRadius, markerRadius / 2,
                    new Color(255, 255, 255, (int)(255 * animation)));

            // Черная обводка
            drawRoundedRectOutline(markerX, markerY, markerRadius, markerRadius, markerRadius / 2,
                    1f, new Color(0, 0, 0, (int)(255 * animation)));
        }
    ...
    public static void drawRotatedRect(double x, double y, double width, double height, double angle, Color color) {
        drawSetup();
        applyColor(color);
        ...
        // Углы прямоугольника до вращения
        double[] cornersX = {x, x + width, x + width, x};
        double[] cornersY = {y, y, y + height, y + height};

        glBegin(GL_QUADS);

        for (int i = 0; i < 4; i++) {
            // Смещаем координаты относительно центра
            double relX = cornersX[i] - centerX;
            double relY = cornersY[i] - centerY;

            // Применяем вращение
            double rotatedX = relX * Math.cos(angle) - relY * Math.sin(angle);
            double rotatedY = relX * Math.sin(angle) + relY * Math.cos(angle);

            // Возвращаем координаты на место
            glVertex2d(centerX + rotatedX, centerY + rotatedY);
        }
